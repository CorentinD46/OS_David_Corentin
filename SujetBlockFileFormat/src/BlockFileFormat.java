...
public class BlockFileFormat {
    ...
    public static BlockFileFormat readFromFile(String filename) throws IOException {
        ...
        for (int sectionPointer : header.sectionPointers) {
            ...
            // Positionner le curseur du fichier à la bonne position
            file.seek(sectionPointer * BLOCK_SIZE);
            //! Lire les données de la section dans le tableau
            ...
            for (int i = 0; i < section.dataPointers.length; i++) {
                ...
                //! Positionner le curseur au bon endroit dans le fichier (pointeur * BLOCK_SIZE)
                file.seek(section.dataPointers[i] * BLOCK_SIZE);
